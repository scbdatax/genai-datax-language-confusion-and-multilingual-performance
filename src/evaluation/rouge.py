# https://github.com/pltrdy/rouge/tree/master
# -*- coding: utf-8 -*-
...
def _split_into_words(sentences, lang):
    """Splits multiple sentences into words and flattens the result"""
    if lang == "en":
        return list(itertools.chain(*[_.lower().split(" ") for _ in sentences]))
    elif lang == "th":
        splitter = Jaccard(method="pythai")
        return list(itertools.chain(*[splitter.split_text(_) for _ in sentences]))
...
def _union_lcs(evaluated_sentences, reference_sentence, lang,
    ...
    reference_words = _split_into_words([reference_sentence], lang)

    combined_lcs_length = 0
    for eval_s in evaluated_sentences:
        evaluated_words = _split_into_words([eval_s], lang)
...
def rouge_l_summary_level(
        evaluated_sentences, reference_sentences, lang, raw_results=False, exclusive=True, **_):
    """
    ...
    m = len(
        Ngrams(
            _split_into_words(reference_sentences, lang),
            exclusive=exclusive))

    # total number of words in evaluated sentences
    n = len(
        Ngrams(
            _split_into_words(evaluated_sentences, lang),
            exclusive=exclusive))
...
def cal_rouge_score(
    ...
    if lang == "th":
        ...
        splitter = Jaccard(method="simple")
        reference = reference.lower()
        candidate = candidate.lower()
    else: NotImplementedError()

    # construct n grams
    ref_ngrams = Counter(ngrams(splitter.split_text(reference), n))
    can_ngrams = Counter(ngrams(splitter.split_text(candidate), n))

    # calculate rouge-n 
    overlap = sum((ref_ngrams & can_ngrams).values())
    if (sum(ref_ngrams.values()) == 0) or (sum(can_ngrams.values()) == 0):
        return 0
    recall = overlap / sum(ref_ngrams.values())
    precision = overlap / sum(can_ngrams.values()) if sum(can_ngrams.values()) > 0 else 0
